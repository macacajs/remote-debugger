  ...
  *start() {
  ...
  configurePort() {
    ...
    const isOk = (devices) => devices.length > 0;
     ...
     // added retry logic because sometimes cannot get simulator devices immediately
  ...
  getPages() {
    const url = `${this.protocol}://${this.host}:${this.port}/json`;
     ...
     // added retry logic because sometimes cannot get pages immediately
    const fetchPages = () => {
      logger.debug(`Getting pages from url: ${url}`);
      return request.get(url).then(JSON.parse);
    };
    const isOk = (pages) => pages.length > 0;

    return _.waitUntil(fetchPages, isOk).then((pages) => {
      return pages
